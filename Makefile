...
macOS: 
	${ELECTRON_PACKAGER} --icon=${ICON_FILE} . Awesom-O --platform darwin --arch x64 --out ${BUILD_DIR}
	${ELECTRON_INSTALLER_DMG} --icon=${INSTALLER_ICON_FILE} \
	  --background=${BACKGROUND_FILE} ${BUILD_DIR}/Awesom-O-darwin-x64/Awesom-O.app/ Awesom-O

linux: 
	${ELECTRON_PACKAGER} . Awesom-O --platform linux --arch x64 --out ${BUILD_DIR} 
	${ELECTRON_INSTALLER_DEBIAN} --src ${BUILD_DIR}/Awesom-O-linux-x64/ --dest . --arch amd64

windows: 
	${ELECTRON_PACKAGER} . Awesom-O --platform win32 --arch x64 --out ${BUILD_DIR} 

all_platforms: clean linux windows macOS
	zip -9 dist/Awesom-O_${NEW_VERSION}_amd64.deb.zip ${BUILD_DIR}/Awesom-O_${NEW_VERSION}_amd64.deb
	zip -9 dist/Awesom-O.dmg.zip ${BUILD_DIR}/Awesom-O.dmg
	zip -r -9 dist/Awesom-O-win32-x64.zip ${BUILD_DIR}/Awesom-O-win32-x64
...
darwin:
	${ELECTRON_PACKAGER} . --overwrite --platform=darwin --arch=x64 --icon=${ICON_FILE} --prune=true --out ${BUILD_DIR} 
